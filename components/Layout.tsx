...
export const Layout: React.FC<LayoutProps> = ({ children, title, onBack, onNavigate, currentView = 'dashboard' }) => {
  return (
    <div className="min-h-screen bg-[#fcfcfc] flex flex-col font-sans selection:bg-black selection:text-white">
      ...
      <header className="bg-zinc-950 text-white sticky top-0 z-50 shadow-md" role="banner">
        <div className="max-w-7xl mx-auto px-5 md:px-10 h-16 md:h-24 flex items-center justify-between">
          <div className="flex items-center gap-4 md:gap-10">
            {onBack ? (
              <button onClick={onBack} className="p-2 md:p-3.5 hover:bg-white/10 rounded-full transition-all group border border-zinc-800 shadow-lg active:scale-90" aria-label="返回上一頁">
                <ChevronLeft size={20} className="md:w-7 md:h-7 group-hover:-translate-x-1 transition-transform" />
              </button>
            ) : (
              // Make logo clickable to go home
              <button onClick={() => onNavigate?.('dashboard')} className="w-10 h-10 md:w-14 md:h-14 bg-white flex items-center justify-center rounded-sm shadow-xl transition-transform hover:rotate-12 cursor-pointer" aria-label="回到首頁">
          ...
          <div className="hidden lg:flex items-center gap-14 text-[10px] font-black tracking-[0.4em] uppercase whitespace-nowrap">
            ...
            <button
              ...
              aria-label="數據中心"
              aria-current={currentView === 'datacenter' ? 'page' : undefined}
            >
              數據中心 / ANALYTICS
            </button>
            <button
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-10 pb-48 md:pb-32" role="main" aria-label={title}>
        {children}
      </main>
...
const NavButton = ({ icon, label, active = false, onClick }: { icon: React.ReactNode, label: string, active?: boolean, onClick?: () => void }) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1 transition-all whitespace-nowrap ${active ? 'text-black scale-105' : 'text-zinc-500 hover:text-black'}`} aria-label={label.split(' / ')[0]} aria-current={active ? 'page' : undefined}>
    {icon}
