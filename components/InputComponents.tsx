...
export const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement> & { label?: string }> =
  ({ label, className = '', children, ...props }) => (
    <div className="flex flex-col gap-1 w-full">
      {label && <label className="text-[9px] font-black text-zinc-400 uppercase tracking-[0.15em] leading-none mb-1 ml-0.5 whitespace-nowrap">{label}</label>}
      <select className={`border border-zinc-200 rounded-sm px-3 py-2 focus:ring-2 focus:ring-zinc-950/5 focus:border-zinc-950 outline-none bg-white text-base font-black text-zinc-900 transition-all shadow-sm ${className}`} {...props}>
        {children}
      </select>
    </div>
  );
