...
const ZoneCard: React.FC<{ zone: Zone; methods: MethodItem[]; onUpdate: (z: Zone) => void; onDelete: () => void }> = ({ zone, methods, onUpdate, onDelete }) => {
  ...
  const updateItem = (iIdx: number, field: string, value: any) => {
    ...
    item.itemPrice = Math.round((zone.unit === '坪' ? item.areaPing : 1) * zone.unitPrice * zone.difficultyCoefficient);
    newItems[iIdx] = item;
      ...
      title={
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[7px] text-zinc-300 uppercase tracking-widest">{zone.methodName || "未選工法"}</span>
          </div>
        </div>
      }
      ...
      <div className={isCollapsed ? 'hidden' : 'space-y-6 animate-in fade-in duration-300'}>
        ...
        <div className="pt-4 border-t border-zinc-50 space-y-4">
          ...
          {zone.items.map((item, iIdx) => (
            <div key={item.itemId} className="p-3 border border-zinc-50 rounded-sm bg-zinc-50/10 space-y-3">
              ...
              <div className="flex justify-between items-end">
                 <div className="text-xl font-black text-zinc-950">{item.areaPing} <span className="text-[10px] text-zinc-400 uppercase ml-1">PING</span></div>
